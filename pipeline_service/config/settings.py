...
class Settings(BaseSettings):
    ...
    # Trellis settings
    trellis_model_id: str = Field(
        default="jetx/trellis-image-large", env="TRELLIS_MODEL_ID"
    )
    trellis_sparse_structure_steps: int = Field(
        default=10,
        env="TRELLIS_SPARSE_STRUCTURE_STEPS",
        description="Increased from 8 for better accuracy",
    )
    trellis_sparse_structure_cfg_strength: float = Field(
        default=5.75, env="TRELLIS_SPARSE_STRUCTURE_CFG_STRENGTH"
    )
    trellis_slat_steps: int = Field(
        default=35,
        env="TRELLIS_SLAT_STEPS",
        description="Increased from 20 for better quality",
    )
    trellis_slat_cfg_strength: float = Field(
        default=2.4, env="TRELLIS_SLAT_CFG_STRENGTH"
    )
    trellis_num_oversamples: int = Field(
        default=8,
        env="TRELLIS_NUM_OVERSAMPLES",
        description="Increased from 3 for better accuracy",
    )
    ...
    qwen_edit_model_path: str = Field(
        default="Qwen/Qwen-Image-Edit-2509", env="QWEN_EDIT_MODEL_PATH"
    )
    ...
    num_inference_steps: int = Field(
        default=10,
        env="NUM_INFERENCE_STEPS",
        description="Increased from 8 for better quality",
    )
    ...
    qwen_edit_megapixels: float = Field(
        default=1.5,
        env="QWEN_EDIT_MEGAPIXELS",
        description="Megapixels for image preprocessing (higher = more detail preserved)",
    )

    # Background removal settings
    background_removal_model_id: str = Field(
        default="hiepnd11/rm_back2.0", env="BACKGROUND_REMOVAL_MODEL_ID"
    )
    input_image_size: tuple[int, int] = Field(
        default=(1024, 1024), env="INPUT_IMAGE_SIZE"
    )  # (height, width)
    output_image_size: tuple[int, int] = Field(
        default=(518, 518), env="OUTPUT_IMAGE_SIZE"
    )  # (height, width)
    padding_percentage: float = Field(default=0.2, env="PADDING_PERCENTAGE")
    limit_padding: bool = Field(default=True, env="LIMIT_PADDING")
    background_mask_threshold: float = Field(
        default=0.8,
        env="BACKGROUND_MASK_THRESHOLD",
        description="Threshold for background mask (0.0-1.0). Higher = more strict",
    )
    ...
    enable_image_preprocessing: bool = Field(
        default=True, env="ENABLE_IMAGE_PREPROCESSING"
    )
